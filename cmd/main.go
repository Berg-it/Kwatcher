...
import (
	...
	"kwatch.cloudcorner.org/k-watcher/internal/controller"
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "2c504800.kwatch.cloudcorner.org",
	...
	if err = (&controller.KwatcherReconciler{
		Client:                mgr.GetClient(),
		Scheme:                mgr.GetScheme(),
		MaxConcurrentRollouts: 3,
		RolloutQueue: workqueue.NewNamedRateLimitingQueue(
			workqueue.NewItemExponentialFailureRateLimiter(5*time.Second, 300*time.Second),
			"rollouts",
		),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "Kwatcher")
		os.Exit(1)
	}
